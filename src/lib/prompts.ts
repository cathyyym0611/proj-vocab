...
2. 每个英语单词第一次出现时，紧跟其后用【中文含义】标注，唯一允许格式为：englishWord【中文含义】
...
7. 不要把单词写成（englishWord）【中文】、【englishWord】【中文】、englishWord [中文]、(englishWord)(中文) 等其他变体；只允许 "englishWord【中文含义】" 这一种格式
...
const STYLE_PROMPTS: Record<StoryStyle, string> = {
  ...
  romance:
    "请严格用现代都市言情/狗血短剧风格写一个名场面，必须发生在当代城市语境中。人物身份应是总裁、律师、医生、明星、前任、未婚夫、闺蜜、豪门继承人等现代角色，场景应是公司、酒店、医院、民政局、晚宴、热搜、朋友圈、直播间等现代场景。可以写豪门恩怨、虐恋情深、误会反转、追妻火葬场等经典桥段。对白要扎心，情节要抓人，像热搜上的爆款都市短剧。禁止出现古代宫廷、皇上、娘娘、王爷、江湖门派、武侠招式等非现代元素。",
  wuxia:
